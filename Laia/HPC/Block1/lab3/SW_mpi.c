...
main (int argc, char *argv[])
{
  ...
  if (!rank) { /* Use process 0 as the master process
  ...
  CHECK_NULL ((s1l = (short *) malloc (sizeof (short) * (dim1 + 1))));
  ...
  dim1local = (rank ? nrows : dim1); /* condition ? true_case : false_case */
  ...
  /* PARALLEL CODE */
  for (int jj = 1; jj <= dim2; jj += BS) {   /* Strip mining: Define blocks of size BS in the columns */

   if (rank != 0) MPI_Recv(&h[0][jj], min(BS, dim2-jj+1), MPI_INT, rank-1, 1, MPI_COMM_WORLD, &status);

   for (i = 1; i <= nrows; i++)
    for (j = jj; j < min(BS+jj, dim2+1); j++) /* Strip mining: Traverse BS columns within the current block */
      {
   ...
   if (rank != nprocs-1 ) MPI_Send(&h[nrows][jj], min(BS, dim2-jj+1), MPI_INT, rank+1, 1, MPI_COMM_WORLD);
  ...
  if ( !rank ) free(s1); /* Only the master process allocated it */
