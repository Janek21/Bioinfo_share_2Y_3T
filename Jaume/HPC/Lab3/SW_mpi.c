...
main (int argc, char *argv[])
{
  ...
  if (! ... ) { /* Use process 0 as the master process
  ...
  /* PARALLEL CODE */
  MPI_Bcast( ... ); /* Broadcast dim1 */
  MPI_Bcast( ... ); /* Broadcast dim2 */
  MPI_Bcast( ... ); /* Broadcast BS */
  MPI_Bcast( ... ); /* Broadcast DELTA */
  MPI_Bcast( ... ); /* Broadcast matrix sim */
  ...
  dim1local = (rank ? nrows : ...); /* condition ? true_case : false_case */
