...
int main(int argc, char **argv) {
    ...
    numreq = 0;
    if (rank < size - 1) 
        MPI_Isend(&xlocal[maxn/P][0], maxn, MPI_DOUBLE, rank + 1, 0, MPI_COMM_WORLD, &r[numreq++]); /* Statement S7 */
    
    /* Receive from previous unless I'm the first process (rank==0)
       and store row in initial row (ghost area) */
    if (rank > 0)
        MPI_Irecv(&xlocal[0][0], maxn, MPI_DOUBLE, rank - 1, 0, MPI_COMM_WORLD, &r[numreq++]); /* Statement S8 */

    /* Send 1st local row to previous process unless I'm the 1st process */
    if (rank > 0) 
        MPI_Isend(&xlocal[1][0], maxn, MPI_DOUBLE, rank - 1, 1, MPI_COMM_WORLD, &r[numreq++]); /* Statement S9 */
    
    /* Receive from next process and store row in last row in xlocal
       (ghost area) unless I'm the last process */
    if (rank < size - 1) 
        MPI_Irecv(&xlocal[maxn/P + 1][0], maxn, MPI_DOUBLE, rank + 1, 1, MPI_COMM_WORLD, &r[numreq++]); /* Statement S10 */

    /* Wait for all four requests with a single call */
    MPI_Waitall(numreq, r, statuses);
    ...
    for (j = 0; j < maxn; j++) {
        ...
        if (rank < size - 1 && xlocal[maxn/P + 1][j] != rank + 1) errcnt++;
    }
