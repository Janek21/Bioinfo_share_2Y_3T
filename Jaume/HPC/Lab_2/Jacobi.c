...
char **argv;
{
	...
	if (size != P) {
	   ...
	   usleep(10); //Pause by 10 microseconds
	...
	do {
		...
		MPI_Allreduce(&diffnorm, &globaldiffnorm, 1, MPI_DOUBLE, MPI_SUM, MPI_COMM_WORLD);
		globaldiffnorm = sqrt(globaldiffnorm);
		if (rank == 0) printf("At iteration %d, diff is %e\n", itcnt, globaldiffnorm);
	...
	MPI_Gather(xlocal[1], maxn * (maxn/P), MPI_DOUBLE, x, maxn * (maxn/P), MPI_DOUBLE, 0, MPI_COMM_WORLD);
	if (rank == 0) {
