...
author: "Marta Coronado-Zamora and Olga Dolgova (current version 04/2024)<br>First version: Marta Coronado-Zamora and Antonio Barbadilla 03/2020"
date: "15 April 2024"
output:
  html_document:
    theme: yeti
...
```{r answer-exercise-1}
# the arguments of this functions are (p_0, Ne, tgen, nrepl)

genetic_drift(0.5, 500, 100, 20) # All alleles show polymorphism
genetic_drift(0.5, 100, 100, 20) # Most alleles show polymorphism with some fixed or lost
genetic_drift(0.5, 10, 100, 20) # Alleles are either fixed or lost, very few are polymorphic
genetic_drift(0.9, 500, 100, 20) # Most alleles have high frequencies with few fixations
genetic_drift(0.9, 100, 100, 20) # Some alleles fixed, others retain polymorphism
genetic_drift(0.9, 10, 100, 20) # All alleles are either fixed or lost with no polymorphic alleles

# Increasing Ne reduces the number of alleles that are fixed or lost; higher effective populations maintain more polymorphism.
# Starting with a higher p_0 results in more alleles having higher frequencies, leading to more fixations and fewer losses.
...
| p0=0.2, N=500 | 0.221  | All polymorphic |
| p0=0.9, N=500 | 0.901  | 1 fixed and 9 polymorphic |
| p0=0.5, N=100 | 0.435  | 1 fixed, 1 lost and 8 polymorphic |                |
| p0=0.2, N=100 | 0.2  | 4 lost, 6 polymorphic |
| p0=0.9, N=100 | 0.916  | 4 fixed, 6 polymorphic |
| p0=0.5, N=10  | 0.614  | 4 lost, 5 fixed, 1 polymorphic |
| p0=0.2, N=10  | 0.294  | 8 lost, 2 fixed|
| p0=0.9, N=10  | 0.885  | All fixed|
...
# Higher initial allele frequencies (p_0) lead to more fixations and generally larger allele frequencies, resulting in a mean value > 0.5
# Lower initial allele frequencies result in more allele losses and generally smaller allele frequencies, resulting in a mean value < 0.5
# An initial p_0 of 0.5 typically results in equal numbers of fixations and losses, with overall frequencies averaging around 0.5

# Increasing the effective population size (Ne) results in more polymorphic alleles and fewer fixations or losses.
# Decreasing Ne leads to a greater number of fixations or losses, which varies based on the initial allele frequencies.
...
# When implementing the bottleneck during generation 50, a significant shift in allele frequencies is observed due to the sudden reduction in population size. This phenomenon is absent when the bottleneck is not introduced.
...
# The population modeled using the harmonic mean more precisely forecasts the effects of a bottleneck. This accuracy arises because the inclusion of even a single very low value significantly affects the overall harmonic mean, a dynamic not observed with the arithmetic mean.
...
# We have an allele frequency of 0.33 when we are in equilibrium.
```
