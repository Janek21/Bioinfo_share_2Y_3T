...
```{r answer-exercise-1}
# Population 1 initial genotypic frequencies
P1 <- 0.24
Q1 <- 0.32
R1 <- 0.44

# Population 2 initial genotypic frequencies
P2 <- 0.33
Q2 <- 0.14
R2 <- 0.53


result_pop1 <- random_mating(P1, Q1, R1)


print("Population 1:")
print(paste("Genotype prob. initial generation (f11, f12, f22): ", result_pop1$f11, result_pop1$f12, result_pop1$f22))
print(paste("Allele prob. next generation (p' and q'): ", result_pop1$p_, result_pop1$q_))
print(paste("Genotype prob. next generation (f'11, f'12, f'22): ", result_pop1$f11_, result_pop1$f12_, result_pop1$f22_))

result_pop2 <- random_mating(P2, Q2, R2)

# Print results for Population 2
print("Population 2:")
print(paste("Genotype prob. initial generation (f11, f12, f22): ", result_pop2$f11, result_pop2$f12, result_pop2$f22))
print(paste("Allele prob. next generation (p' and q'): ", result_pop2$p_, result_pop2$q_))
print(paste("Genotype prob. next generation (f'11, f'12, f'22): ", result_pop2$f11_, result_pop2$f12_, result_pop2$f22_))
...
# Modified function to have assortative mating
assortative_mating <- function(f11, f12, f22, mating_type = "positive_genotype") {
   
   p = f11 + f12/2 # allele frequency of X1
   q = 1 - p # allele frequency of X2
   

   if (mating_type == "positive_genotype") {
     ...
     f12_22 = 0
     f22_22 = f22^2
   } else if (mating_type == "positive_phenotype") {
     ...
     f12_22 = 0
     f22_22 = (f12/2 + f22)^2
   } else if (mating_type == "negative_genotype") {
     ...
     f12_22 = f12 * f22
     f22_22 = 0
   } else if (mating_type == "negative_phenotype") {
     ...
     stop("Invalid mating type. Please choose from 'positive_genotype', 'positive_phenotype', 'negative_genotype', or 'negative_phenotype'.")
   }
   
   # Genotype probabilities of offspring f11_, f12_, f22_
   f11_ = f11_11 + f11_12/2 + f12_12/4
   f12_ = f11_12/2 + f11_22 + f12_12/2 + f12_22/2
   f22_ = f12_12/4 + f12_22/2 + f22_22
...
result_positive_genotype <- assortative_mating(0.24, 0.32, 0.44, mating_type = "positive_genotype")
print("Positive assortative mating among genotypes:")
print(paste("Genotype prob. next generation (P'1, Q'1, R'1):", result_positive_genotype$f11_, result_positive_genotype$f12_, result_positive_genotype$f22_))

# Positive assortative mating among phenotypes
result_positive_phenotype <- assortative_mating(0.24, 0.32, 0.44, mating_type = "positive_phenotype")
print("Positive assortative mating among phenotypes:")
print(paste("Genotype prob. next generation (P'1, Q'1, R'1):", result_positive_phenotype$f11_, result_positive_phenotype$f12_, result_positive_phenotype$f22_))

# Negative assortative mating among genotypes
result_negative_genotype <- assortative_mating(0.24, 0.32, 0.44, mating_type = "negative_genotype")
print("Negative assortative mating among genotypes:")
print(paste("Genotype prob. next generation (P'1, Q'1, R'1):", result_negative_genotype$f11_, result_negative_genotype$f12_, result_negative_genotype$f22_))

# Negative assortative mating among phenotypes
result_negative_phenotype <- assortative_mating(0.24, 0.32, 0.44, mating_type = "negative_phenotype")
print("Negative assortative mating among phenotypes:")
print(paste("Genotype prob. next generation (P'1, Q'1, R'1):", result_negative_phenotype$f11_, result_negative_phenotype$f12_, result_negative_phenotype$f22_))
```
...
navajo_results <- HWE_test(305, 52, 4)


print(paste("Sample size = ", navajo_results$N, " -> N11 = ", navajo_results$N11, ", N12 = ", navajo_results$N12, ", N22 = ", navajo_results$N22))
print(paste("p = ", round(navajo_results$p, digits = 3), " and q = ", round(navajo_results$q, digits = 3)))
print(paste("f11 = ", round(navajo_results$f11, digits = 3), ", f12 = ", round(navajo_results$f12, digits = 3), ", f22 = ", round(navajo_results$f22, digits = 3)))
print(paste("Chi-square value = ", round(navajo_results$Chi_value, 3), "; Prob. = ", round(navajo_results$prob, 3)))

# Proportion of children of women of phenotype N expected to present the maternal phenotype
proportion_maternal_NN <- navajo_results$N22 / navajo_results$N
print(paste("Proportion of children of women of phenotype N expected to present the maternal phenotype: ", round(proportion_maternal_NN, 3)))

# Proportion of children of heterozygous MN women expected to have the maternal phenotype
proportion_maternal_MN <- navajo_results$N12 / (2 * navajo_results$N)
print(paste("Proportion of children of heterozygous MN women expected to have the maternal phenotype: ", round(proportion_maternal_MN, 3)))
...
HWE_test_ABC <- function(AA, AB, AC, BB, BC, CC) { 

   total <- AA + AB + AC + BB + BC + CC # total sample
   
   # Allele frequencies estimation
   p = (AA + AB/2 + AC/2) / total  #frequency of allele A
   q = (BB + AB/2 + BC/2) / total  # frequency of allele B
   r = 1 - p - q  # frequency of allele C
   ...
   E_BC <- 2 * q * r * total
   E_CC <- r^2 * total
   SUM = (E_AA + E_AB + E_AC + E_BB + E_BC + E_CC) /total
   
   # Chi-square value and probability
   Chi_value = (AA-E_AA)^2/E_AA + (AB-E_AB)^2/E_AB + (AC-E_AC)^2/E_AC + (BB-E_BB)^2/E_BB + (BC-E_BC)^2/E_BC + (CC-E_CC)^2/E_CC 
   
   prob = pchisq(Chi_value, df = 4, lower = F)

   # Output
   return (list("total" = total, "p" = p, "q" = q, "SUM" = SUM,
                "Chi_value" = Chi_value, "prob" = prob))
}

# Invoke the function
results4 <- HWE_test_ABC(17,86,5,61,9,0)

# Print results
print(paste("Frequency of allele A =", results4$p, ", frequency of allele B =", results4$q, ", frequency of allele C =", results4$r))
print(paste("Deviation =", results4$Chi_value, " and significance =", results4$prob))
print("The data is not with the HWE")
...
# Calculate the frequency of the recessive allele (c)
frequency_cf <- 1/1700

# Invoke the function
results <- freq_dominance((1 - frequency_cf) * 200, frequency_cf * 200)

print(paste("Percent of population with cystic fibrosis: ", 1/1700 * 100))
print(paste("Frequency of recessive allele in the population: ", round(results$q, digits = 6)))
print(paste("Frequency of dominant allele in the population: ", round(results$p, digits = 6)))
print(paste("Percentage of heterozygous individuals (carriers) in the population: ", round(results$f12 * 100, digits = 2), "%"))
...
# Calculate the frequency of albino phenotype (aa)
frequency_albino <- 1/10000

# Invoke the function
results <- freq_dominance((1 - frequency_albino) * 200, frequency_albino * 200)

# Print the frequency of the allele produced by albinism
print(paste("Frequency of allele produced by albinism: ", round(results$q, digits = 6)))
print(paste("Proportion of non-albino individuals who are carriers: ", round(results$f12 * 100, digits = 2),"%"))
...
plot(generations, p,type = "l", col = "blue", xlab = "Generation", ylab = "Allele Frequency", main = "Change in Allele Frequency")

#The results from this analysis differ from the previous one because we're considering the random mating under Hardy-Weinberg Equilibrium (HWE), where allele frequencies remain constant over generations.
...
plot(generations, p, type = "l", col = "blue", xlab = "Generation", ylab = "Allele Frequency", main = "Change in Allele Frequency of A (p) over Generations")
plot(generations, q, type = "l", col = "red", xlab = "Generation", ylab = "Allele Frequency", main = "Change in Allele Frequency of a (q) over Generations")


#Frequencies tend to an equilibrium around 0.37 and 0.63
```
