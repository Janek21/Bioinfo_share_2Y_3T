...
main (int argc, char *argv[])
{
  ...
  MPI_Bcast( sim, AA, MPI_INT, 0, MPI_COMM_WORLD ); /* Broadcastº matrix sim */ /// AA is size of matrix

  nrows = getRowCount( AA, rank, nprocs); ///getRowCount(int rowsTotal, int mpiRank, int mpiSize)
  ...
  CHECK_NULL ((s1l = (short *) malloc ((short) * (&nrows )))); ///& beacuse if not its not an integer
  ...
  MPI_Scatter( &s1+1, nrows, MPI_SHORT, s1l+1, nrows, MPI_SHORT, 0, MPI_COMM_WORLD); ///send buf, count, type, recv buf, count, type, root (data origin), comm
   ...
   *
   *       */
  MPI_Bcast( &s2+1, dim2, MPI_SHORT, 0, MPI_COMM_WORLD); ///ni idea
  ...
  dim1local = (rank ? nrows : dim1); /* condition ? true_case : false_case */ ///if rank!=0 dim1local=nrowa(local row count), else: dim1local= length of 1rst sequence(dim1)
  ...
  for (int jj = 1; jj <= dim2; jj += BS) {   /* Strip mining: Define blocks of size BS in the columns */ /// jj+=BS because jumps of size BS
   ...
   if (rank != 0) MPI_Recv( &h[0][jj], min(BS, dim2-jj+1), MPI_INT, rank-1, 0, MPI_COMM_WORLD, &status);	/// falta comment

   for (i = 1; i <= nrows ; i++) /// falta comment
    for (j = jj; j < min(jj+BS, dim2); j++) /* Strip mining: Traverse BS columns within the current block */ ///choose the smallest BS column, com les aconseguim? /// falta comment
      { ////missing +1 above (dim2+1)
   ...
   if (rank != nprocs-1 ) MPI_Send( &h[nrows][jj], min(BS, dim2-jj+1), MPI_INT, rank+1, 0, MPI_COMM_WORLD); ///send it
   /* Here we could use a non-blocking send. */ ///MPI_Isend+MPI_wait /// to add later
  }
  ...
  if ( rank==0 ) free(s1); /* Only the master process allocated it */ ///check if rank is 0
