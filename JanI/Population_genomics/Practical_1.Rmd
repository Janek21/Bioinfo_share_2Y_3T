...
author: "Marta Coronado Zamora and Marta Puig (current version 04/2023)<br>First version: Marta Coronado-Zamora and Antonio Barbadilla 03/2020 <br> Solved version: Jan Izquierdo Ramos"
date: "8 April 2024"
output:
  html_document:
    theme: yeti
...
```{r answer-exercise-1}
cat("Offsprings of population 1\n")
result1 <- random_mating(0.24, 0.32, 0.44)

paste("Genotype prob. initial generation (f11, f12, f22): ", result1$f11, result1$f12, result1$f22)
paste("Allele prob. next generation (p' and q'): ", result1$p_, result1$q_)
paste("Genotype prob. next generation (f'11, f'12, f'22): ", result1$f11_, result1$f12_, result1$f22_)


cat("Offsprings of population 2\n")
result2 <- random_mating(0.33, 0.14, 0.53)

paste("Genotype prob. initial generation (f11, f12, f22): ", result2$f11, result2$f12, result2$f22)
paste("Allele prob. next generation (p' and q'): ", result2$p_, result2$q_)
paste("Genotype prob. next generation (f'11, f'12, f'22): ", result2$f11_, result2$f12_, result2$f22_)
```
...
ProbAllMatings = result$f11*result$f11 + result$f12*result$f12 + result$f22*result$f22

#self combinations
f11_11 = (result$f11*result$f11)/ProbAllMatings

f12_12 = (result$f12*result$f12)/ProbAllMatings

f22_22 = (result$f22*result$f22)/ProbAllMatings

#mixed combinations
f11_12 = (result$f11*result$f12)/ProbAllMatings
f11_22 = (result$f11*result$f22)/ProbAllMatings

f12_22 = (result$f12*result$f22)/ProbAllMatings
#f12_11 has already been calculated (f11_12), and 22 for all as well

##????? #pq f11_11 = f11_12 i f12_12=f12_22 i f22_22=f11_22 ????
f11_11 = result$f11*result$f22/ProbAllMatings
f11_12 = 0
f11_22 = 0
f12_12 = result$f12*result$f22/ProbAllMatings
f12_22 = 0
f22_22 = result$f11*result$f22/ProbAllMatings
...
navajo_res<-HWE_test(305, 52, 4)

#Test for the deviation from HWE and its significance.
cat("Sample size = ", navajo_res$N," -> N11 = ", navajo_res$N11,", N12 = ", navajo_res$N12,",N22 = ", navajo_res$N22)
cat("\np = ",round(navajo_res$p,digits=3)," and q = ", round(navajo_res$q,digits=3))
cat("\nf11 = ",round(navajo_res$f11,digits=3),", f12 = ",round(navajo_res$f12,digits=3)," and f22 = ", round(navajo_res$f22,digits=3))
cat("\nChi-square value =",round(navajo_res$Chi_value,3),"; Prob. = ",  round(navajo_res$prob,3))


#What proportion of children of women of phenotype N are expected to present the maternal phenotype?
proportion_NN <- (navajo_res$f22)^2
cat("\nThe proportion of children of women of phenotype N that are expected to present the maternal phenotype is", round(proportion_NN, 4))

#What proportion of children of heterozygous MN women are expected to have the maternal phenotype?
proportion_MN <- 2 * navajo_res$f12 * navajo_res$f22
cat("\nThe proportion of children of heterozygous MN women that are expected to have the maternal phenotype is", round(proportion_MN, 3))
...
```{r answer-exercise-4}
HWE_allele_test_3<-function(N11, N12, N13, N22, N23, N33){ #AA, AB, AC, BB, BC, CC
  N=N11+N12+N13+N22+N23+N33
  
  #Freq estimation
  p = (N11 + 0.5 * N12 + N13) / (2 * N)
  q = (N22 + 0.5 * N12 + N23) / (2 * N) 
  r = 1 - p - q
  ...
  chi_v = (N11 - E11)^2 / E11 + (N12 - E12)^2 / E12 + (N13 - E13)^2 / E13 + (N22 - E22)^2 / E22 + (N23 - E23)^2 / E23 + (N33 - E33)^2 / E33 
  prob= pchisq(chi_v, df = 3, lower = F)
  
  res=2
  
  return(list("N"=N, "N11" = N11, "N12" = N12, "N13" = N13, "N22" = N22, "N23" = N23, "N33" = N33, "f11" = f11, "f12" = f12, "f13" = f13, "f22" = f22, "f23" = f23, "f33" = f33, "p"=p, "q"=q, "r"=r, "Chi_v" = chi_v, "probability"=prob))
  
}

test_result<-HWE_allele_test_3(17, 86, 5, 61, 9, 0)

print(paste("Allele frequencies: A =", round(test_result$p, 3), ", B =", round(test_result$q, 3), ", C =", round(test_result$r, 3)))
print(paste("Chi-square value = ", round(test_result$Chi_v, 3), "; Probability = ", round(test_result$probability, 3)))
...
res <- freq_dominance(0.19 * 200, (1-0.19)*200) #dominant, recessive

#Heterozygotes Aa are in f12, so res$f12
cat("Heterozygote frequency: ", round(res$f12, 3), "\n")
...
res<-freq_dominance((1-0.09)*200, 0.09 * 200)
...
res<-freq_dominance((1 - cy_fi_freq)*200, cy_fi_freq*200) #recessive have the disease

cat("A", round(cy_fi_freq*100, 3),"% of the population will have cystic fibrosis\n")
cat("Recessive allele frequency:", round(res$q, 3))
cat("\nDominant allele frequency:", round(res$p, 3))
cat("\nA", round(res$f12*100, 3),"% of the population are heterozygous\n")
...
res<-freq_dominance((1 - alb_freq)*200, alb_freq*200) #recessive have the disease

cat("Frequency of allele produced by albinism:", round(res$q, 3))

cat("\nProportion of non-affected carriers:", round(res$f12*100, 2))
```
...
```{r, F1.1}
random_mating_new<-function(f11, f12, f22){
  ...
  return (list("p" = p, "q" = q, "f11" = f11, "f12" = f12, "f22" = f22, "p_" = p_, "q_" = q_, "f11_" = f11_, "f12_" = f12_, "f22_" = f22_))
}

res_f<-random_mating_new(0.5, 0, 0.5)
print(paste("Allele prob. next generation (p' and q') ",res_f$p_, res_f$q_))
print(paste("Genotype prob. next generation (f'11, f'12, f'22) ",res_f$f11_, res_f$f12_, res_f$f22_))
```
...
f22<-0.5
for (i in 1:25) {
res_l<-random_mating_new(f11, f12, f22)
f11<-res_l$f11_
f12<-res_l$f12_
f22<-res_l$f22_
}
cat("In 25 generations, the allele frequencies will be: \nf11:", f11, "\nf12:", f12, "\nf22:", f22)
cat("In 25 generations, the genotype frequencies will be: \np:", res_l$p_, "\nq:", res_l$q_)
```
...
for (i in 1:25) {
res_l<-random_mating(f11, f12, f22)
f11<-res_l$f11_
f12<-res_l$f12_
f22<-res_l$f22_
p[i] <- res_l$p
...
random_mating_dual<-function(f11, f12, f22, pm, pf){
  
  p = f11 + f12/2 + pm/2
  q = 1-p
  
  prob = (pm * f11) + (2 * pm * f12) + (pm * f22) + (2* pf * f12) + (pf * f22) + (pf * f11)
  f1_11 = pm * f11 / prob
  f1_12 = 2 * pm * f12 / prob
  f1_22 = pm * f22 / prob
  f2_12 = 2* pf * f12 / prob
  ...
  pm_ = f1_12/4 + f1_11/2 + f2_12/4 + f2_11/2
  pf_ = f1_12/4 + f1_22/2 + f2_12/4 + f2_22/2
  # Allele probability of progeny p_ and q_
  p_ = f11_ + f12_/2 + pm_/2
  q_ = 1 - p_
  # Output
  return (list("p" = p, "q" = q, "f11" = f11, "f12" = f12, "f22" = f22, "p_" = p_, "q_" = q_, "f11_" = f11_, "f12_" = f12_, "f22_" = f22_, "pm_" = pm_, "pf_" = pf_))
}
...
for (i in 1:25) {
res_d<-random_mating_dual(f11, f12, f22, pm, pf)
f11<-res_d$f11_
f12<-res_d$f12_
f22<-res_d$f22_
pm<-res_d$pm_
...
plot(p, col="blue", pch=19)
plot(q, col="red", pch=19)


cat("We can see that both frequencies tend to an equilibrium, to prove it further\np:", p[25], "\nq:", q[25], "\np+q should be 1  \np+q=", p[25]+q[25], "\nThere is an equilibrium")
```
