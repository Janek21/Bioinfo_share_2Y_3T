...
# Convert 'cohort' to a factor and create a design matrix
cohort <- as.factor(se$cohort)
design <- model.matrix(~0 + cohort)
colnames(design) <- levels(cohort)
design

# Convert 'race' to a factor and include it in the design matrix along with 'age' and 'cohort'
race <- as.factor(se$race)
age <- se$age
design <- model.matrix(~0 + cohort + age + race)

# Correct column names for the design matrix
...
# Copy dgl object to dgl2
dgl2 <- dgl

# Drop rows with NA values in 'entrezid'
dgl2 <- dgl2[!is.na(dgl2$genes$entrezid), ]

# Drop rows with duplicated 'entrezid'
dgl2 <- dgl2[!duplicated(dgl2$genes$entrezid), ]

# Drop rows where 'entrezid' contains spaces indicating multiple IDs
dgl2 <- dgl2[!grepl(" ", dgl2$genes$entrezid), ]

# Set the row names of dgl2 to the 'entrezid'
rownames(dgl2) <- as.character(dgl2$genes$entrezid)


# Apply the 'voom' transformation to the data with the specified design matrix, no plotting
y2 <- voom(dgl2, design, plot = FALSE)

# Fit the linear models for each gene using the design matrix
fit2 <- lmFit(y2, design)

# Apply the contrasts to the fitted models
fit2 <- contrasts.fit(fit2, contr.matrix)

# Use empirical Bayes statistics to enhance the fit
fit2 <- eBayes(fit2)
