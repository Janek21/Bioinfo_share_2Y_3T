...
```{r 1.fill and save the matrix, results=FALSE}
...
annotation_col <- data.frame( Tissue = filtered_metadata$V2, Period = filtered_metadata$PERIOD, row.names = filtered_metadata$UniqueName)
m <- as.matrix(m)
...
#E071 on filtered -> Name in UniqueNames -> m[name1] is row of name -> m[name1, name2] is row and column of names
...
#Create a column with the rownames, for easier joining with filtered_metadata (needed for PERIOD and tissue)
...
ggplot(df_multi_d_scaling, aes(x = MDS1, y = MDS2, color = V2, shape = PERIOD))+geom_point(size = 5)+ labs(x="Dimension 1", y="Dimension 2")
```

Plot for dimensions 3 and 4
```{r 5.Plot dimensions 3 and 4}
ggplot(df_multi_d_scaling, aes(x = MDS3, y = MDS4, color = V2, shape = PERIOD))+geom_point(size = 5)+ labs(x="Dimension 3", y="Dimension 4")
```
