...
#POPULATION 1
# Invoke the function
result_1 <- random_mating(0.24, 0.32, 0.44)

# Print results
print(paste("Genotype prob. next generation (f'11, f'12, f'22), for population 1, are: ",result_1$f11_, result_1$f12_,result_1$f22_))


#POPULATION 2
# Invoke the function
result_2 <- random_mating(0.33,0.14,0.53)

# Print results
print(paste("Genotype prob. next generation (f'11, f'12, f'22), for population 2, are: ",result_2$f11_, result_2$f12_,result_2$f22_))
...
```{r answer-exercise-2}
# Complete assortative positive mating among phenotypes (a given genotype only mates with its same phenotype)
random_mating_1 <- function(f11, f12, f22) {
...
# Complete assortative negative mating among genotypes (a given genotype never mates with its same genotype and mate at random with other genotypes)
random_mating_2 <- function(f11, f12, f22) {
...
# Complete assortative negative mating among phenotypes (a given phenotype never mates with its same phenotype)
random_mating_3 <- function(f11, f12, f22) {
...
result_1<- random_mating_1(0.24, 0.32, 0.44)
result_2<- random_mating_2(0.24, 0.32, 0.44)
result_3<- random_mating_3(0.24, 0.32, 0.44)

# We output the genotypic frequencies of the progeny for the 1st scenario
print(paste("In the 1st escenario, the genotypic frequencies of the next generation, are: f11 =", result_1$f11_, "f12 =", result_1$f12_, "f22 =", result_1$f22_))
# We output the genotypic frequencies of the progeny for the 2nd scenario
print(paste("In the 2nd escenario, the genotypic frequencies of the next generation, are: f11 =", result_2$f11_, "f12 =", result_2$f12_, "f22 =", result_2$f22_))
# We output the genotypic frequencies of the progeny for the 3rd scenario
print(paste("In the 3rd escenario, the genotypic frequencies of the next generation, are: f11 =", result_3$f11_, "f12 =", result_3$f12_, "f22 =", result_3$f22_))
```
...
```{r answer-exercise-3}
#Test:
HWE_test_1 <- function(N11, N12, N22) {
   N = N11 + N12 + N22 # N total sample
...
result_test <- HWE_test_1(305, 52, 4)
print(paste("Deviation =", result_test$Chi_value, "Significance =", result_test$prob))

#Proportion of children of women of phenotype N expected to present the maternal phenotype
random_mating_Women_N <- function(f11, f12, f22) {
   
   # Mating pairs (frequencies) probabilities
   ProbAllMatings = (f11 * f22) + (f12 * f22) + (f22 * f22)
   f11_22 = f11 * f22 / ProbAllMatings
   f12_22 = f12 * f22 / ProbAllMatings
   f22_22 = f22 * f22 / ProbAllMatings

   # Genotype probabilities of progeny (offspring) f22_
   f22_ = f12_22/2 + f22_22

   # Output
   return (list("f22_" = f22_))
}

result_Women_N <- random_mating_Women_N(result_test$f11, result_test$f12, result_test$f22)
print(paste("Expected proportion of children of women of phenotype N that present the maternal phenotype  =", result_Women_N$f22_))

#Proportion of children of heterozygous MN women expected to have maternal phenotype.
random_mating_Women_MN <- function(f11, f12, f22) {

   # Mating pairs (frequencies) probabilities
   ProbAllMatings = (f11 * f12) + (f12 * f12) + (f12 * f22)
   f11_12 = f11 * f12 / ProbAllMatings
   f12_12 = f12 * f12 / ProbAllMatings
   f22_12 = f12 * f22 / ProbAllMatings
   
   # Genotype probabilities of progeny (offspring) f12_
   f12_ = f11_12/2 + f12_12/2 + f22_12/2
   
   # Output
   return (list("f12_" = f12_))
}

result_Women_MN <- random_mating_Women_MN(result_test$f11, result_test$f12, result_test$f22)
print(paste("Expected proportion of children of women of phenotype MN that present the maternal phenotype  =", result_Women_MN$f12_))
...
# Create the function
HWE_test_ABC <- function(AA, AB, AC, BB, BC, CC) { #in this case we have six genotypes and three alleles
...
print("The data is not consistent with the HWE")
...
# What is the frequency of heterozygotes Aa in a population with random mating in which the frequency of the dominant phenotype is 0.19?
# Create the function
freq_heterozygotes <- function(NA_, Naa) {
    
    # Allele frequency
    q = sqrt(Naa)
    p = 1 - q;
    
    # Genotype frequencies according HWE, heterozygotes
    f12 = 2*p*q
...
NA_ = 0.19
Naa = 1 - NA_
result_ex5 <- freq_heterozygotes(NA_, Naa)
print(paste("The frequency of heterozygotes is", result_ex5))
```
...
NA_ = 1 - Naa
Naa = 0.09
result_ex6 <- freq_heterozygotes(NA_, Naa)
print(paste("The frequency of heterozygotes is", result_ex6))
```
...
```{r answer-exercise-7}
result_ex7 <- freq_dominance(1699, 1)
print(paste("The percent of cystic fibrosis is", (result_ex7$f22)*100, "%.", "The frequency of the recessive allele is", result_ex7$q, ".", "The frequency of the dominant allele is", result_ex7$p, ".", "The percentage of carriers is", (result_ex7$f12)*100, "%."))
```
...
```{r answer-exercise-8}
#What proportion of non-albino individuals are carriers?
results_ex8 <- freq_dominance(9999, 1)
print(paste("The frequency of the allele a is", results_ex8$q, "and the proportion carriers out of non-albino individuals is", ((results_ex8$f12)*10000)/9999))
```
...
random_mating <- function(f11, f12, f22) {
   ...
   f22_22 = f22 * f22
   # Genotype probabilities of progeny (offspring) f11_, f12_, f22_
   f11_ = f11_11 + 2*f11_12/3 + 4*f12_12/9
   f12_ = f11_12/3 + f11_22 + 4*f12_12/9 + 2*f12_22/3
   f22_ = f12_12/9 + f12_22/3 + f22_22
   # Allele probability of progeny p_ and q_
   p_ = f11_ + f12_/2
   q_ = 1 - p_
   # Output
...
result_final1 <- random_mating(0.5, 0, 0.5)
print(paste("Allele frequencies:", result_final1$p_, result_final1$q_, "Genotype frequencies:", result_final1$f11_, result_final1$f12_, result_final1$f22_))
...
f22 <- 0.5
for (i in 1:25) {
result_9 <- random_mating(f11, f12, f22)
f11 <- result_9$f11_
f12 <- result_9$f12_
f22 <- result_9$f22_
}
print("Expected allele and generotype frequencies over 25 generations:")
print(paste("f11:", f11, "f12:", f12, "f22:", f22))
print(paste("p_:", result_9$p_, "q_:", result_9$q_))
...
for (i in 1:25) {
  result_10 <- random_mating(f11, f12, f22)
  f11 <- result_10$f11_
  f12 <- result_10$f12_
  f22 <- result_10$f22_
...
- Perform the same  but using the original `random_mating()` function, that assumes that alleles equally segregate, using the same initial frequencies: `f11=0.5`, `f12=0`, `f22=0.5`. What pattern do you observe? Is it the same as before?
...
plot(generations, p)
#We can observe that the result is very different to the previously one, this is because we are in hwe, and the frequencies are equal than the previously analysis.
...
# We can clearly see that both frequencies tend to a certain equilibrium. 
#   - p = 0.3746532
#   - q = 0.6253468
```
