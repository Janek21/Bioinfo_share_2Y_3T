...
# Mating pairs (frequencies) probabilities for complete assortative positive mating among phenotypes
...
# Mating pairs (frequencies) probabilities for complete assortative negative mating among genotypes
ProbAllMatings = result$f11*result$f11 + result$f12*result$f12 + result$f22*result$f22
f11_11 = 0
...
# Mating pairs (frequencies) probabilities for complete assortative negative mating among phenotypes
ProbAllMatings = result$f11*result$f11 + result$f12*result$f12 + result$f22*result$f22
...
# Invoke the function with Navajo population genotype data
...
print(paste("f11 = ",round(navajo_results$f11,digits=3),", f12 = ",round(navajo_results$f12,digits=3)," and f22 = ", round(navajo_results$f22,digits=3)))
...
proportion_NN <- (navajo_results$f22)^2
print(paste("Proportion of children of women of phenotype N expected to present the maternal phenotype: ", round(proportion_NN, 3)))

# Proportion of children of heterozygous MN women expected to have the maternal phenotype
proportion_MN <- 2 * navajo_results$f12 * navajo_results$f22
print(paste("Proportion of children of heterozygous MN women expected to have the maternal phenotype: ", round(proportion_MN, 3)))
...
HWE_test_3alleles <- function(N11, N12, N13, N22, N23, N33) {
  ...
  # Chi-square value and probability
  Chi_value = (N11 - E11)^2 / E11 + (N12 - E12)^2 / E12 + (N13 - E13)^2 / E13 +
    (N22 - E22)^2 / E22 + (N23 - E23)^2 / E23 + (N33 - E33)^2 / E33
  prob = pchisq(Chi_value, df = 3, lower = F)

  return(list("N" = N, "N11" = N11, "N12" = N12, "N13" = N13,
              "N22" = N22, "N23" = N23, "N33" = N33,
              "p" = p, "q" = q, "r" = r,
              "f11" = f11, "f12" = f12, "f13" = f13,
              "f22" = f22, "f23" = f23, "f33" = f33,
              "Chi_value" = Chi_value, "prob" = prob))
}


results_3alleles <- HWE_test_3alleles(17, 86, 5, 61, 9, 0)


print(paste("Sample size = ", results_3alleles$N))
print(paste("Allele frequencies: A =", round(results_3alleles$p, 3), ", B =", round(results_3alleles$q, 3), ", C =", round(results_3alleles$r, 3)))
print(paste("Genotype frequencies: AA =", round(results_3alleles$f11, 3), ", AB =", round(results_3alleles$f12, 3), ", AC =", round(results_3alleles$f13, 3), ", BB =", round(results_3alleles$f22, 3), ", BC =", round(results_3alleles$f23, 3), ", CC =", round(results_3alleles$f33, 3)))
print(paste("Chi-square value = ", round(results_3alleles$Chi_value, 3), "; Prob. = ", round(results_3alleles$prob, 3)))
