...
main (int argc, char *argv[])
{
  ...
  for (int jj = 1; jj <= dim2; jj += BS) {   /* Strip mining: Define blocks of size BS in the columns */
   ...
   if (rank != 0) MPI_Recv(&h[0][jj], min(BS,dim2-jj+1), MPI_INT, rank-1, 1,
		   MPI_COMM_WORLD, &status );

   for (i = 1; i <= nrows ; i++)
    for (j = jj; j < min(jj+BS,dim2+1); j++) /* Strip mining: Traverse BS columns within the current block */
      {
   ...
   if (rank != nprocs-1 ) MPI_Send(&h[nrows][jj], min(BS,dim2-jj+1), MPI_INT, rank+1, 1,
		   MPI_COMM_WORLD);
