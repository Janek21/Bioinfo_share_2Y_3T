...
```{r answer-exercise-1}
dominant_allele_selection <- selection(0.005, c(1, 1, 1-0.05), 5000)
cat("If A is rare and dominant, the initial increase is rapid since we see an increase from the first generations, so this prediction is true.\n\n")

recessive_allele_selection <- selection(0.005, c(1, 1-0.05, 1-0.05), 5000)
cat("If A is rare and recessive, the initial increase is very slow since we start to see an increase when nearly 4000 generations have passed, so this prediction is true.\n\n")

deleterious_never_eliminated <- selection(0.005, c(1-0.05, 1-0.05, 1-0.05), 5000)
cat("Deleterious alleles are completely eliminated with the effect of time, so this prediction is false.")
```
...
selection_coef <- selection(0.005, w, 52)

cat("Approximately the selection coefficient is s=0.18 (1-s=0.82, s=1-0.82, s=0.18), since the frequency of p is roughly 0.776 after 52 generations with a fitness value of 0.82. As the fitness value increases, the frequency of p decreases and viceversa.")
...
# Yes, the effect can be reversible since it is an ability of the population to revert back to a previous state under certain conditions. In our case, the M allele was advantageous in a polluted environment but as this became cleaner, the allele was less frequent. If the environment ever became polluted again (by not using electrical/non-polluting energy resources), the M allele frequency would increase, so we can confirm that the effect of evolutionary change due to natural selection is reversible. 
...
results <- drift.selection(p0 = 0.005, w = c(w_AA,w_Aa,w_aa), ngen =52 , Ne =100 , nrep=20)

cat("There are cases in this simulation of the melanic allele being lost, pretty much the whole population. 18/20 are lost, so 9 out of 10 individuals is the proportion. \n\n")

cat("Given my fitness level of 0.82, the selection coefficient is 0.18, which according to Fisher’s hypothesis, predicts a survival probability of 36%. However, the survival rate observed in my case is only 10% (only 2 out of 20 alleles survived), which is less than the predicted value.")
```
...
children_counts <- c(AA = 189, AS = 89, SS = 9)
adult_counts <- c(AA = 400, AS = 249, SS = 5)

children_freq <- children_counts / sum(children_counts)
adult_freq <- adult_counts / sum(adult_counts)

children_HW_test <- HWChisq(children_counts)
adult_HW_test <- HWChisq(adult_counts)

print(paste("Children genotype frequencies: ", children_freq))
print(paste("Adult genotype frequencies: ", adult_freq))
print(paste("Hardy-Weinberg equilibrium test for children: ", children_HW_test$pval))
print(paste("Hardy-Weinberg equilibrium test for adults: ", adult_HW_test$pval))

cat("\nThe frequency of the AS genotype is higher in adults (0.3807) than in children (0.3101). This could suggest that individuals with the AS genotype have an advantage of survival because they might be resistent to malaria. \n\n")

cat("No, the genotypic frequencies in children do not differ from those expected in Hardy-Weinberg equilibrium. The p-value for the Hardy-Weinberg equilibrium test in children is 0.8272, which is greater than 0.05. This means that we accept the null hypothesis that the population is in Hardy-Weinberg equilibrium. \n\n")

cat("Yes, the genotypic frequencies in adults differ from those expected in Hardy-Weinberg equilibrium.The p-value for the Hardy-Weinberg equilibrium test in adults is approximately 6.38e-07, which is under 0.05. This means that we reject the null hypothesis that the population is in Hardy-Weinberg equilibrium. \n\n")

cat("The significant deviation from Hardy-Weinberg equilibrium in adults suggests that selection might be favoring the AS genotype, possibly due to its resistance to malaria.\n")
...
| Number of children    | 189   | 89    | 9     |
| Number of adult       | 400   | 249   | 5     | 
| Frequency in children | 0.659 | 0.310 | 0.031 |
| Frequency in adults   | 0.612 | 0.381 | 0.007 |
| Relative survival     | 0.929 | 1.228 | 0.244 |
| Relative fitness      | 0.756 | 1     | 0.199 |
| Selective coefficients (s<sub>1</sub> and s<sub>2</sub>)  | 0.244    |  0   | 0.801   |


```{r answer-exercise-6}
# Frequencies calculated in Ex 5
relative_survival <- adult_freq / children_freq
relative_fitness <- relative_survival / relative_survival["AS"]

s1 <- 1 - relative_fitness["AA"]
s2 <- 1 - relative_fitness["SS"]

cat("Relative survival\t", relative_survival["AA"], "\t", relative_survival["AS"], "\t", relative_survival["SS"], "\n")
cat("Relative fitness\t", relative_fitness["AA"], "\t", relative_fitness["AS"], "\t", relative_fitness["SS"], "\n")
cat("Selective coefficients (s1 and s2)\t", s1, "\t", "N/A", "\t", s2, "\n")
```
...
results_500 <- drift.selection(p0 = p0, w = w, ngen = ngen, Ne = 500, nrep = nrep)

results_100 <- drift.selection(p0 = p0, w = w, ngen = ngen, Ne = 100, nrep = nrep)

results_10 <- drift.selection(p0 = p0, w = w, ngen = ngen, Ne = 10, nrep = nrep)


cat("We can conclude that the bigger the group, the stronger the influence of selection is. This allows the A allele to persist at different frequencies within the population. However, in small groups (N=10), genetic drift can outweigh the effects of selection which leads to the fixation of the A allele. This shows that selection can maintain a variety of genes in larger populations (even if the heterozygotes are favored), but in smaller populations genetic drift can eliminate this variety, even against strong selection.\n")
...
In this practical, i explored how allele frequencies can change over time due to forces like
selection and genetic drift. I discovered that in large populations, selection can maintain genetic diversity, even when one genotype has a strong advantage. However, in smaller populations, genetic drift can lead to a loss of this diversity. I also got hands-on experience with R by simulating these processes and interpreting the results.
