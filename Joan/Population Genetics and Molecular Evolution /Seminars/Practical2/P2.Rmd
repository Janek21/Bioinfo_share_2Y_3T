...
```{r answer-exercise-1}
genetic_drift(0.5, 500, 100, 20)
genetic_drift(0.5, 100, 100, 20)
genetic_drift(0.5, 10, 100, 20)
genetic_drift(0.9, 500, 100, 20)
genetic_drift(0.9, 100, 100, 20)
genetic_drift(0.9, 10, 100, 20)

#When the initial frequency is 0.9 the chances of the a allele disappearing are higher and it does it in earlier generations. This same thing happens with decreasing the population, where we can see that the graphs with population 10 don't conserve both alleles in any replicate. On the 100 population most have both alleles but as said before the one with the 0.9 frequency has more replicas with no a allele. And finally, on the 500 population, the frequency 0.5 will hardly have any replica with alleles disappearing and the one with the 0.5 is incredibly unlikely for a replica to go even below the 0.5 frequency mark
...
| p0=0.2, N=500 | 0.2    | All polymorphic |
| p0=0.9, N=500 | 0.876  | 9 polymorphic |
| p0=0.5, N=100 | 0.458  | 9 polymorphic |
| p0=0.2, N=100 | 0.122  | 4 polymorphic |
| p0=0.9, N=100 | 0.877  | 4 polymorphic |
| p0=0.5, N=10  | 0.747 | None polymorphic |
| p0=0.2, N=10  | 0.126 | None polymorphic |
| p0=0.9, N=10  | 0.921 | None polymorphic |
...
| Simulation | A is lost (p=0) | Polymorphic |  A is fixed (p=1) |
|:-------------:|:---------------:|:-----------:|:-----------------:|
| p0=0.5, N=1000, with bottleneck | 0 | All are polymorphic | 0 |
| p0=0.5, N=1000, without bottleneck | 0 | All are polymorphic | 0 |

```{r answer-exercise-3}
bottleneck <- function(p_0, Ne, tgen, nrepl) {
  
  plot(0,0, xlim=c(0,tgen+1), ylim = c(0,1), 
       col="coral2", type = "l", lwd = 3,
       main = paste("Bottleneck"), xlab="Generations" , ylab = "Relative frequency")
  meanpvector <- 0*c(1:(tgen+1))
  meanpvector[1] <- p_0
  for (n in 1:nrepl) {
    pvector <- c()
...
#Since the population was 1000 in all generations except the 50th one which was 10, so the vector should mirror it.
popVector<-c(rep(Ne, 99), 10)
popHarm<-round(harmMean(popVector))

#harmonic drift
genetic_drift(p_0, popHarm, tgen, nrepl)
...
mut_dyn <- function(p0, mu, tgen) {
    ...
    cat("After 50000 generations p will be", 1-freqvector[i+1])
    lines(freqvector, type = "l",lwd=3, col="deepskyblue4")

}
mut_dyn(0.8,0.000004,50000)


equilibriumA<- 0.000004/(0.0000006+0.000004)

cat("The allele frequency will be", equilibriumA, "for A and", 1-equilibriumA, "for a" )
```
